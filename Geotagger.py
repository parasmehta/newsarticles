...
def load_data():
    ...
    # load articles
    f_articles_london = open('articles_london.txt', 'rb')       # open file
...
def geotagging_streets(article):
    return False

def geotagging_adminlevels(article):
    return False

def timetagging(article):
    return False

# helper functions
def getTopic(article):
    return ""
...
def main():
    ...
    for article in data:
        ...
        # GEOTAGGING
        if (not isGeotagged):
            isGeotagged = geotagging_streets(article)           # find streets mentioned in article
        if (not isGeotagged):
